...
int main() {
    ...
    vector<double> normalized_inputs = Data(inputs).dataset_normalization(NORMTYPE::SYMMETRIC); // Normalized input
    double target = Data(inputs).targetdata_normalization(NORMTYPE::SYMMETRIC, 3.25); // Expected output
    ...
    */
    ...
    Neuron neuron(
        ...
        HIDDEN, UNIFORM, LEAKY_RELU, ITDECAY, ADAM, MSE
    );

    neuron.set_step_size(step_size);
    neuron.set_target(target); // Set target value for training
    neuron.set_dropout(0.6); // Set dropout rate for regularization
