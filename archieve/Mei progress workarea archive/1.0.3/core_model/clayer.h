...
class Layer{
    private:
        ...
        vector<double> input, bias, gradient, gradient_bias, output, activated_output, 
    ...
    public:
          ...
          : input(inputs),
            ...
            ntype(ntype), distribution(distribution), actFunc(actfunc), lr_schedule(lr_schedule), opt(opt), lossFunc(lossFunc) {
                ...
                for (size_t i = 0; i < num_neuron; ++i) {
                    ...
                    ntype, distribution, actFunc, lr_schedule, opt, lossFunc
                );
        ...
        void gradient_hlayer(vector<unique_ptr<Neuron>> neuron){
            for(size_t i = 0; i < neurons.size(); i++){
                double gradient_sum = 0.0;

                for(size_t j = 0; j < neuron.size(); j++){
                    for(size_t k = 0; k < neuron[j] -> get_weight().size(); k++){
                        gradient_sum += neuron[j] -> get_gradient() * neuron[j] -> get_weight()[k];
                    }
                }
