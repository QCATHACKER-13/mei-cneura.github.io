...
public:
    Neural(const vector<size_t>& num_neurons, const vector<double>& inputs,
           ...
           DISTRIBUTION distribution, vector<ACTFUNC> actFunc, LEARNRATE lr_schedule, OPTIMIZER opt, LOSSFUNC lossFunc) 
           : 
           num_neurons(num_neurons) {
            ...
            for (size_t i = 0; i < num_neurons.size(); ++i) {
                ...
                auto layer = make_unique<Layer>(
                    ...
                    layer_type, distribution, actFunc[i], lr_schedule, opt, lossFunc
                );
    ...
    void learning(int steps, double threshold = 0.05, int report_every = 0, int patience = 0) {
        ...
        for (int step = 1; step <= steps; ++step) {
            ...
            if (current_loss < best_loss - 1e-8) {
                ...
                if (patience > 0 && patience_counter >= patience) {
                    std::cout << "Early stopping: no improvement for " << patience << " steps.\n";
                    break;
                }
            }

            if (report_every > 0 && step % report_every == 0) {
                std::cout << "Step " << step << "/" << steps << " | Loss: " << current_loss << std::endl;
            }
            
            if (below_threshold) {
                std::cout << "Learning completed at step " << step << " (loss: " << current_loss << ")\n";
                break;
            }
    ...
    void enforce_learning(int epochs, double threshold = 0.05, int report_every = 0, int patience = 0) {
        if (layers.empty()) throw invalid_argument("The layers didn't initialize properly.");
        auto start = high_resolution_clock::now();
        ...
        for (int epoch = 1; epoch <= epochs; ++epoch) {
            ...
            for (int step = 1; step <= epoch; ++step) {
                ...
                if (below_threshold) {
                    std::cout << "Early stopping at step " << step << " (Epoch " << epoch << "/" << epochs << ") due to error threshold.\n";
                    goto finished;
                }
                
                if (report_every > 0 && step % report_every == 0) {
                    std::cout << "Epoch " << epoch << ", Step " << step << " | Loss: " << current_loss << std::endl;
                }
                
                backpropagation();
                regularization();
            }
            std::cout << "Epoch " << epoch << " completed. Average Loss: " << (epoch_loss / epoch) << std::endl;
        }
        finished:
        print();
        auto stop = high_resolution_clock::now();
