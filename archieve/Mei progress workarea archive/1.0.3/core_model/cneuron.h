...
class Neuron {
    ...
    public:
        ...
        Neuron(vector<double> inputs, 
            ...
            NEURONTYPE ntype, DISTRIBUTION distribution, ACTFUNC actFunc, LEARNRATE lr_schedule, OPTIMIZER opt, LOSSFUNC lossFunc)
            
            : input(inputs),
            learning_rate(learn_rate), decay_rate(decay_rate), beta(beta),
            ntype(ntype), distribution(distribution), actFunc(actFunc), lr_schedule(lr_schedule), opt(opt), lossFunc(lossFunc)
        ...
        void update_weights() {
            ...
            for (size_t i = 0; i < weight.size(); ++i) {
                ...
                gradient_weight[i] =  -(error * activation_derivative(output) * max(min(dropout_backpropagation(activation_value(output)), 1.0), -1.0) * input[i]) + error_Lw[i]; // Gradient clipping
        ...
        void update_bias(){
            ...
            gradient_bias = -(error* activation_derivative(output)+
                dropout_backpropagation(activation_value(output))) + error_Lb;// Gradient clipping
