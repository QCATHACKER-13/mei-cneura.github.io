...
class Layer{
    private:
        ...
        double decay_rate, loss, avg_error; // Decay rate for learning rate, loss function
    ...
    public:
        ...
        void regression_loss_function() {
            ...
            for(size_t i = 0; i < neurons.size(); ++i) {
                this->neurons[i]->set_loss(this->loss); // Set loss for each neuron
            }
        }
        ...
        void classification_loss_function() {
            ...
            #pragma omp parallel for reduction(+:sum_loss)
            for (size_t i = 0; i < neurons.size(); ++i) {
                this -> loss += neurons[i]->get_loss(); // Initialize loss from first neuron
                switch (lossFunc) {
                    case MSE:
                    ...
                    case CCE:
                        sum_loss += -(probability_target[i] * log(clamp(activated_output[i], EPSILON, 1.0 - EPSILON)));
                        break;
            ...
            for (size_t i = 0; i < neurons.size(); i++) {
                ...
                switch (lossFunc) {
                    ...
                    case HUBER: {
                        ...
                        if (abs(d) <= delta_thresh) delta_h = d;
                        else delta_h = delta_thresh * ((d > 0) ? 1.0 : -1.0);
        ...
        void backpropagation(){
            ...
            for(size_t i = 0; i < neurons.size(); i++){
                neurons[i] -> backward();
                this -> loss += (neurons[i] -> get_loss())/neurons.size();
        ...
        void set_passing_loss(double& loss) {
            this->loss = loss;
            for (size_t i = 0; i < neurons.size(); ++i) {
                neurons[i]->set_loss(this -> loss);
            }
