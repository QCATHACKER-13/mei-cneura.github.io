...
class Neuron {
    private:
        ...
        inline static std::mt19937& global_rng() {
        ...
        inline double dropout(double x){
        ...
        /*
        inline double dropout_backpropagation(double x){
        ...
        */
        ...
        inline double activation_value(double x) {
            switch (actFunc) {
    ...
    public:
        ...
        void feedforward() {
            ...
            this -> output = dropout(inner_product(input.begin(), input.end(), weight.begin(), bias));

            //assert(!isnan(output)); // Check for NaN output
            this -> activated_output = activation_value(output);
            //this->activated_output = dropout(activation_value(output));
            //this->activated_output = clamp(activation_value(output), -1.0, 1.0);
            //this->activated_output = clamp(dropout_feedforward(activation_value(output)), -1.0, 1.0); // Clamp output to prevent extreme values
        ...
        void update_weights() {
            ...
            for (size_t i = 0; i < weight.size(); ++i) {
                ...
                this -> loss += ((lambda_weight[i] * pow(weight[i], 2)) + (lambda_weight[i] * copysign(1.0, weight[i])))/weight.size();
                this -> error_Lw[i] = (2 * lambda_weight[i] * weight[i]) + (lambda_weight[i]);
                this -> gradient_weight[i] = (gradient * input[i]) + error_Lw[i];
                //this -> gradient_weight[i] = clamp((gradient * input[i] * dropout(1)) + error_Lw[i], 1.0, -1.0);
        ...
        void update_bias(){
            ...
            this -> loss += (lambda_bias * pow(bias, 2)) + (lambda_bias * copysign(1.0, bias));
            this -> error_Lb = (2 * lambda_bias * bias) + (lambda_bias);
            this -> gradient_bias = gradient + error_Lb;
            //this -> gradient_bias = clamp((gradient * dropout(1)) + error_Lb, -1.0, 1.0);
        ...
        void set_loss(double& loss) {this -> loss = loss;} //Setting the loss of the neuron
