...
int main() {
    ...
    vector<double> targets = {3.0, 0.0, 0.0}; // Single target for the output layer
    ...
    Layer hiddenLayer(3, inputs, biasRange, weightRange, targets, learningRate, actFunc);

    // Perform feedforward for hidden layer
    hiddenLayer.feedforward();

    // Create an output layer with 1 neuron
    Layer outputLayer(3, hiddenLayer.getOutput(), biasRange, weightRange, targets, learningRate, actFunc);

    // Perform feedforward for output layer
    outputLayer.feedforward();

    // Print neuron details of the output layer
    cout << "Output Layer after initial feedforward:" << endl;
    ...
    outputLayer = Layer(3, hiddenLayer.getOutput(), biasRange, weightRange, targets, learningRate, actFunc);

    // Perform feedforward again for output layer
    outputLayer.feedforward();

    // Print neuron details of the output layer
    cout << "Output Layer after backpropagation and second feedforward:" << endl;
    outputLayer.measurement();
